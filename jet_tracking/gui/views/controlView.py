...
class GraphsWidget(QFrame, Graphs_Ui):
    def __init__(self, context, signals):
        ...
        self.graphWorker = GraphsThread(context, signals)
        self.graphWorker.moveToThread(self.thread)
        #self.signals.buffers.connect(lambda buff: self.receive(self.buffers, buff))
        #self.signals.avevalues.connect(lambda ave: self.receive(self.averages, ave))
        #self.signals.calibration_value.connect(self.calibrate)
        self.signals.graph_start.connect(self.graphWorker.long_run)
        self.signals.graph_stop.connect(self.thread.requestInterruption)
        self.thread.start()


class GraphsThread(QObject):
    def __init__(self, context, signals):
        super(GraphsThread, self).__init__()
        self.signals = signals
        self.context = context
        self.calibration_values = {}
        self.thread_options = {}
        self.buffers = {}
        self.averages = {}
        self.signals.threadOp.connect(self.options)
        self.signals.buffers.connect(lambda buff: self.receive(self.buffers, buff))
        self.signals.avevalues.connect(lambda ave: self.receive(self.averages, ave))
        self.signals.calibration_value.connect(self.calibrate)
    ...
    def long_run(self):
        while not self.thread().isInterruptionRequested():
            if self.buffers != {} and self.averages != {}:
                self.refresh_plots()
            time.sleep(1/self.thread_options["sampling rate"])           

    def options(self, op):
        self.thread_options = op
        print(self.thread_options)

    def calibrate(self, cal):
        self.calibration_values = cal
        if self.calibrated:
            self.refresh_cal_plots()
        self.calibrated = True

    def receive(self, dtu, vals):
        dtu = vals

    def refresh_plots(self):
        self.plot_data(self.buffers)
        self.plot_ave_data(self.averages)

    def refresh_cal_plots(self):
...
class ControlsWidget(QFrame, Controls_Ui):
    ...
    def _start(self):
        ...
        self.bttn_start.clicked.connect(self.graph_start)

    def graph_start(self):
        self.worker_status.start()
        self.signals.graph_start.emit()

    def _stop(self):
        self.signals.graph_stop.emit()
