...
class graphDisplay(object):
    ...
    def create_plots(self, *args):
        ...
        self.scatter = pg.ScatterPlotItem(pen=pg.mkPen(width=5, color='r'),
                                          size=1)
        self.graph1.addItem(self.scatter)
        self.scatterave = pg.PlotCurveItem(pen=pg.mkPen(width=1, color='w'),
    ...
    def moving_average(self, data):
        ...
        if len(data[3]) % self.nsamp == 0:
            ...
            # should get at least one data point
            avep1 = mean(self.skimmer('low_intensity',
                                      self.intensityRatio[-self.nsamp:]))
    ...
    def event_flagging(self, data):
        '''
        is the gas detector intensity within 2sigma below the mean of
        calibrated i0 if so then flag it as a dropped shot

        if not dropped shot but the downstream intensity is low then it's a
        missed shot I/I0 falls below nsigma of the calibrated mean I/I0 if so
        then flag it as a missed shot skimmer will write a new array that will
        exclude the dropped shots generalize it so that you can differentiate
        based on any input - skim by whatever key you give it (dropped shots or
        missed shots)
        '''
        ...
        # should make sure this is above 0
        if self.intensityRatio[-1] < (self.calibration.get_ratio() -
                                      self.sigma*self.calibration.get_sigma()):
