...
'''
    def _search(motor, intensity, absolute, left, right, tol, nsamp):
        ...
        if i1 < intensity and i2 < intensity:
            signals.message.emit(f'neither direction was better!: {intensity} '
                                 f'> {i1} and {i2}\n'
                                 f'left position:  {left_p} \n'
                                 f'right position:  {right_p} \n')
            return (_search(motor, intensity, absolute, right, tol, nsamp))
        if i1 < i2:
            signals.message.emit(f'i1<i2:  {i1} < {i2}\n'
                                 f'left position:  {left_p} \n'
                                 f'right position:  {right_p} \n')
            return (_search(motor, intensity, left_p, left_third, right, tol,
                            nsamp))
        else:
            signals.message.emit(f'i1>i2:  {i1} > {i2}\n'
                                 f'left position:  {left_p} \n'
                                 f'right position:  {right_p} \n')
            return (_search(motor, intensity, right_p, right_third, left, tol,
                            nsamp))
    ...
    def _search(self, absolute, left, right, tol, nsamp):
        ...
        if i1 < i2:
            ...
            self.signals.message.emit(f"i1 < i2\n"
                                      f"left position: {left_p}  i1: {i1} \n"
                                      f"right position: {right_p} i2: {i2} \n")
    ...
    def backlash_scan(self, left, right, tol, nsamp):
        ...
        if i1 < i2:  # shot 1 is i1 and shot 2 is i2
            ...
            for i in range(5):
                i += 1
                self.motor.move(tol, wait=True)
                self.moves.append([(self.motor.position,
                                    self.average_int(nsamp))])
